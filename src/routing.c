...
static Pipe* local_routing_pipe_client;
static Pipe* local_routing_pipe_server;
...
void ask_for_file() {
    ...
    int retour = 0;
    do {
        retour = read_pipe(local_routing_pipe_client->id_out, file_name); //Reading the request from the pipe (client to routing)
    } while (retour <= 0);
    ...
    write_pipe(local_routing_pipe_server->id_in, request); //Sending the request to the data process

}

void get_back_data_from_data() {
    char* text = (char*) malloc(BUFFER_SIZE);
    read_pipe(local_routing_pipe_server->id_out, text); //Reading the data from the data process
    write_pipe(local_routing_pipe_client->id_in, text);
}

void ini_routing(Pipe* id_pipe1, Pipe* id_pipe2) {
    local_routing_pipe_client = id_pipe1;
    local_routing_pipe_server = id_pipe2;
}
